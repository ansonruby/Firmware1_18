...
Verificar_Invitados     = lib.Control_Archivos.Verificar_Invitados
Buscar_administrador_Invitados = lib.Control_Archivos.Buscar_administrador_Invitados
Buscar_ID_Invitados     = lib.Control_Archivos.Buscar_ID_Invitados
Estado_Invitacion       = lib.Control_Archivos.Estado_Invitacion
...
Actializar_Invitados    =  lib.Control_Ethernet.Actializar_Invitados
...
def Intentos_Actualizar_Usuarios(Cantidad):
    ...
    for Intentos in range(Cantidad):
        ...
        if actualizar_usuarios_por()==1:
            #print 'actualizando invitaciones'
            #print Tiempo()
            Actializar_Invitados(Tiempo())  # actializacion de invitados
            #print 'fin'
            break
        else:
            if PP_Mensajes:
...
def Decision_Torniquete (Res, QR, ID2, Ti,Qr_Te, I_N_S ):
    ...
    global Estados
    #"tipo 4 para invitaciones"#


    #print Qr_Te
    #print ID2

    if Qr_Te == '4'	:	Co = QR+'.'             #QR invitado
    elif Qr_Te == '1'	:	Co = QR+'.'             #QR
    ...
    if Res == 'Access granted-E':
        ...
        if I_N_S == '1':
            if Qr_Te == '4':    Escrivir_Enviar(Co+Ti+'.'+'1.1.'+I_N_S)
            else:               Escrivir_Enviar(Co+Ti+'.'+Qr_Te+'.1.'+I_N_S)
        ...
        if puntos == 3:
            ...
            if (T_A > fin) :
                ...
                if Resp == 'Access granted-E':
                    ...
                    Decision_Torniquete (Resp,QR,"",T_A,'1','1')
        elif puntos == 4:#--------- invitados ---------------------------------------
            if COMODIN == 1:
                SQR = QR.split('.')
                #print SQR[0]

                if SQR[0] == '3':
                    if PP_Mensajes: print 'tipo 3 Denegacion por tipo'

                    Decision_Torniquete ('Denegar',QR,"",T_A,'1','1')
                else:
                    if PP_Mensajes: print 'Invitaciones validaciones'
                    ...
                    if (inicio <= T_A) and (T_A <= fin):
                        if PP_Mensajes: print 'Dentro del rango permitido'
                        #print SQR[1]
                        #print SQR[4]
                        #print Buscar_administrador_Invitados(SQR[1])
                        if Buscar_administrador_Invitados(SQR[1]) == 1:
                            #print 'tiene administrador'
                            #verificacion de id de Invitacion
                            #print Buscar_ID_Invitados(SQR[4])
                            if Buscar_ID_Invitados(SQR[4]) == 1:
                                print 'Esta la invitacion'
                                #verificar direccion de la invitacion
                                Direccion_invi = Estado_Invitacion(QR+'.'+T_A+'.1.0.1')
                                if Direccion_invi== 0: Decision_Torniquete ('Access granted-E', QR, "", T_A, '4', '1')#mejorar entradas y salidas de invitaciones nuevo pero para invitados
                                else:                  Decision_Torniquete ('Access granted-S', QR, "", T_A, '4', '1')#mejorar entradas y salidas de invitaciones nuevo pero para invitados
                            else:
                                if PP_Mensajes: print 'No Esta la invitacion'
                                Decision_Torniquete ('Denegar',QR,"",T_A,'1','1')
                            ...
                            if PP_Mensajes: print 'El administrador no existe'
                            Decision_Torniquete ('Denegar',QR,"",T_A,'1','1')

                    else:
                        #posoble validacion de salida
                        if PP_Mensajes: print 'Invitacion Denegadano no esta en el rango'
                        Decision_Torniquete ('Denegar',QR,"",T_A,'1','1')


                    #Decision_Torniquete ('Access granted-E', QR, "", T_A, '1', '1')#mejorar entradas y salidas de invitaciones
