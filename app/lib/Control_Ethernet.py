...
Escrivir_Archivo_Json   = Ca.Escrivir_Archivo_Json
Funcion_Actualizar_Invitaciones = Ca.Funcion_Actualizar_Invitaciones
...
CE_rl =[        "/api/access/keyboard_access",      # Enviar Teclado
                ...
                "/api/access/get_granted_invitations_pi"      # Peticion Usuarios invitados
        ]
...
#-----------------------------------------------
#ID_Tarjeta         = 'ABDE16022022b827eb0e1979000030' # dispositivo CF_COL_30
...
#-----------------------------------------------
def Peticion_Usuarios_Activos_Invitados(T_actual):

        global ID_Tarjeta
        global CE_V

        #return '{"ID_management":["h1y1","g2yd343","g2yd3ny","h17dm"],"Invitation_ID":["1","2","3","11568"]}' #trama correcta
        #return '{"ID_management":["h1y1","g2yd343","g2yd3ny"],"Invitation_ID" "1","2","3"]}' #danada
        #return '{"ID_manageme":["h1y1","g2yd343","g2yd3ny"],"Invitation_ID":["1","2","3"]}' #trama con nombre incorecto
        ...
        if CE_V != 0 : CE_url = IP_servidor+CE_rl[7]
        else		 : CE_url = IP_servidorP+CE_rl[7]
...
#---------------------------- actualisacion de Tabla_Invitados
def Actializar_Invitados(T_actual):
    Respuesta = Peticion_Usuarios_Activos_Invitados(T_actual)
    if P_Mensajes:  print Respuesta

    if Respuesta.find("Error") == -1:
        try :
            JSON_Respuesta = json.loads(Respuesta)
            #print 'estructua corescta 1'
            JSON_Respuesta
            #print 'estructua corescta 2'
            JSON_Respuesta["ID_management"]
            #print 'estructua corescta 3'
            JSON_Respuesta["Invitation_ID"]
            #print 'estructua corescta 4'
            #--------------------------------
            # ----- actualizando
            Funcion_Actualizar_Invitaciones(JSON_Respuesta)
            #-----------------------------------
            if P_Mensajes:  print 'Actualizo correctamente'
            return 'Invitados Actualizados'
        except Exception as e:
            #except :
            #print e
            if P_Mensajes:  print  'Error :Json formato erroneo'
            return 'Error :Json formato erroneo'
    else:
         if P_Mensajes:  print  'Error : del servidor'
...
#print Actializar_Invitados(T)

"""

Respuesta = Peticion_Usuarios_Activos_Invitados(T)
print Respuesta
...
if Respuesta.find("Error") == -1:
    try :
        ...
        print  'Error :Json formato erroneo'
        #return 'Error :Conection Dominio Listado'
else:
